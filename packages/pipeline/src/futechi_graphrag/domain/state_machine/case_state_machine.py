...
from futechi_graphrag.domain.value_objects.enums import CaseStatus, ConfirmationType
...
class StateEvent:
    ...
    # Wajib diisi hanya jika confirmation_type == ConfirmationType.SICK --
    # dipakai Case.resolve() untuk membuka mitigasi/referensi medis yang
    # spesifik ke penyakit yang dikonfirmasi (lihat case.py).
...
def transition(case: Case, event: StateEvent) -> Case:
    ...
    if event.type == StateEventType.USER_CONFIRMED:
        ...
        case.resolve(
            ...
            event.confirmed_by,
            event.now,
            confirmed_condition=event.confirmed_condition,
        )
