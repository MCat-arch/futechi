...
class Cage:
    ...
    def needs_safety_net_escalation(self, threshold: int | None = None) -> bool:
        return (
            self.status == CageStatus.COOLDOWN and should_force_escalate(
                self.anomaly_count_during_cooldown, threshold
            )
        )
