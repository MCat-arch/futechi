...
Status selalu disimpan dalam bentuk nilai enum `CaseStatus` (huruf kecil,
mis. "confirmed_sick") -- input huruf besar/enum dinormalisasi di sini
supaya semua pembanding di hilir konsisten.
"""
...
RESOLVED_CASE_STATUSES = frozenset(
    {
        CaseStatus.CONFIRMED_SICK.value,
        CaseStatus.CONFIRMED_NOT_SICK.value,
        CaseStatus.CONFIRMED_HEALTHY.value,
    }
)


def normalize_case_status(status: str | CaseStatus) -> str:
    """'CONFIRMED_SICK' / CaseStatus.CONFIRMED_SICK / 'confirmed_sick' -> 'confirmed_sick'."""
    raw = status.value if isinstance(status, CaseStatus) else str(status)
    try:
        return CaseStatus(raw.strip().lower()).value
    except ValueError as exc:
        raise ValueError(f"Status case tidak dikenal: {status!r}") from exc
...
class CaseStore:
    ...
    def upsert_case(
        ...
        *,
        ...
        visual_features: Iterable[str] | None = None,
        environment_conditions: Iterable[str] | None = None,
    ) -> dict[str, object]:
        """
        Simpan/perbarui case. `visual_features` & `environment_conditions`
        (nama canonical, dipakai retrieval ulang di chat) dipertahankan dari
        record lama jika tidak diisi -- sehingga update status saja tidak
        menghapusnya.
        """
        normalized_status = normalize_case_status(status)
        existing = self._cases.get(case_id, {})

        if resolved_at is None and normalized_status in RESOLVED_CASE_STATUSES:
        ...
        record = {
            ...
            "visual_features": (
                list(visual_features)
                if visual_features is not None
                else list(existing.get("visual_features", []))
            ),
            "environment_conditions": (
                list(environment_conditions)
                if environment_conditions is not None
                else list(existing.get("environment_conditions", []))
            ),
        }
