...
def _frame(features=(), others=(), usable=True, reason=None, bird_visible=True) -> FrameExtractionResponse:
...
def test_frame_without_visible_bird_is_discarded_even_with_features() -> None:
    client = FakeMLLMClient(
        [
            _frame(features=[("bloody_feces", 0.75)], bird_visible=False),
            _frame(features=[("conjunctivitis", 0.9)]),
        ]
    )
    result = extract_frame_features([b"red-square", b"bird"], client, ONTOLOGY.visual_feature_catalog(["bird", "feces_litter"]))

    assert result.frames_usable == 1
    assert result.notes == ["frame-0 tidak dapat dinilai: ayam tidak terlihat"]
    assert [c.label for c in result.candidates] == ["conjunctivitis"]
