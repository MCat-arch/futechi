...
def test_chat_prompt_reveals_mitigation_and_treatment_only_when_confirmed_sick() -> None:
    candidate = replace(
        CRD, medical_treatments=[RawMedicalTreatment("[DUMMY] Obat CRD", "[DUMMY] dosis", "[DUMMY] 7 hari")]
    )
    context = GraphContext(candidates=[candidate])

    pending = build_chat_prompt(context, None, "pending_confirmation", None, [])
    confirmed = build_chat_prompt(context, None, "confirmed_sick", CRD.disease_name, [])

    assert "pemeriksaan yang disarankan (dari graph)" in pending and "refer_pcr_test" in pending
    assert "isolate_affected_bird" not in pending and "[DUMMY] Obat CRD" not in pending
    assert "isolate_affected_bird [prioritas high]" in confirmed
    assert "[DUMMY] Obat CRD: dosis=[DUMMY] dosis, withdrawal_period=[DUMMY] 7 hari" in confirmed
    assert "[DUMMY]" in CHAT_SYSTEM_PROMPT
