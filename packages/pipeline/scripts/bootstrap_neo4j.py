...
Usage:
    python scripts/bootstrap_neo4j.py                 # terapkan migrasi yang belum pernah jalan
    python scripts/bootstrap_neo4j.py --reset --yes   # KOSONGKAN database lalu terapkan semua migrasi
"""
...
load_dotenv(_discover_env_file())
...
def bootstrap(reset: bool = False) -> None:
    ...
    Args:
        reset: Jika True, kosongkan seluruh isi database sebelum migrasi.
    ...
    with driver.session() as session:
        if reset:
            print("RESET: menghapus seluruh node dan relasi (termasuk _SchemaMigration)...")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Bootstrap schema & seed Neo4j.")
    parser.add_argument(
        "--reset",
        action="store_true",
        help="Kosongkan SELURUH isi database sebelum menerapkan migrasi.",
    )
    parser.add_argument(
        "--yes",
        action="store_true",
        help="Konfirmasi wajib untuk --reset.",
    )
