...
OpenAI-compatible, sehingga endpoint bisa diarahkan ke OpenCode, Z.ai,
vLLM self-host, dsb. lewat LLM_BASE_URL tanpa mengubah kode.
"""
...
class LLMClient(Protocol):
    ...
    def generate(self, system_prompt: str, user_prompt: str) -> str:
        ...
        for _ in range(self.max_repair_attempts + 1):
            raw = self._complete(model, message, json_mode=self._json_mode)
        response = self._client.chat.completions.create(**kwargs)
        if not response.choices:
            raise LLMResponseFormatError("Respons LLM kosong")
        choice = response.choices[0]
        if getattr(choice, "finish_reason", None) == "length":
            raise LLMResponseTruncatedError("Respons LLM terpotong karena melebihi max_tokens")
        content = choice.message.content
        if not content:
            #TODO: isi apa gitu
            raise LLMResponseFormatError("content kosong")
...
class OpenAICompatibleLLMClient:
    ...
    def _structured(
        ...
        for _ in range(self._max_repair_attempts + 1):
            raw = self._complete(model, messages, json_mode=self._json_mode)
            try:
...
def build_llm_client(settings: Settings | None = None) -> OpenAICompatibleLLMClient:
    ...
    return OpenAICompatibleLLMClient(
        ...
        multimodal_model=settings.mllm_model,
...
class LLMResponseTruncatedError(LLMResponseFormatError):
    """Respons LLM terpotong karena melebihi batas max_tokens."""
