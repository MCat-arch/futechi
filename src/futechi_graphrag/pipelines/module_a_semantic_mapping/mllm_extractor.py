...
Aturan:
1. Masukkan tanda ke `features` HANYA jika terlihat jelas pada gambar, dan `name` WAJIB persis salah satu nama dari daftar fitur yang diberikan.
2. Tanda abnormal yang terlihat tetapi tidak ada padanannya di daftar dimasukkan ke `other_observations` dengan label deskriptif singkat.
3. `confidence` (0.0-1.0) menyatakan seberapa yakin tanda itu benar-benar terlihat, BUKAN seberapa parah.
4. `evidence` berisi deskripsi singkat bagian gambar yang menunjukkan tanda tersebut.
5. Jangan menyebut nama penyakit di field mana pun.
6. Jangan menebak tanda pada area yang tidak terlihat (tertutup, di luar frame). Jika tidak ada tanda abnormal, kembalikan daftar kosong.
7. Jika gambar tidak dapat dinilai (terlalu buram, gelap, atau ayam tidak terlihat), set `image_usable` = false dan isi `unusable_reason`.
"""
...
def extract_frame_features(
    ...
    for index, frame in enumerate(frames):
        ...
        if not response.image_usable:
            notes.append(
                f"{frame_id} tidak dapat dinilai: {response.unusable_reason or 'tanpa alasan'}"
            )
            continue
